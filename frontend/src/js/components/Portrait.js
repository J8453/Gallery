    ...
    return (
        <div className="info__portrait">
            <div className="info__portrait--coverContainer">
                ...
                { isLoggedIn && owner==currentUser && <div className="info__portrait--cover mask" onClick={maskCallback}>{maskText}</div> }
            </div>
            <div className="info__portrait--introduction">
                { isLoggedIn && owner==currentUser && <span className="editIcon pointer" onClick={editCallback}>🖊</span> }
                <h1 className="info__portrait--title">{title}</h1>
                <p className="info__portrait--description">{description}</p>
            </div>
            { isLoggedIn && owner==currentUser &&
